            ...
            try {
                ...
                for (let i = 0; i < foundRooms.length; i++) {
                    ...
                    if (foundRooms[i].topic === this.roomTopic && foundRooms[i].users.length < 4) {
                        ...
                        yield index_1.default.Room.update({ users: updatedRoomUsers }, { where: { id: foundRooms[i].id } });
                        +(yield index_1.default.User.update({ roomId: foundRooms[i].id }, { where: { id: this.user.id } }));
                        return true;
                    }
                    ;
                }
