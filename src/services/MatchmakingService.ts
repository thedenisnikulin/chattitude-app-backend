...
export default class Matchmaking {
    ...
    public async findRoom(): Promise<boolean> {
        try {
            ...
            let foundRooms: Array<RoomModel> = await db.Room.findAll({
            ...
            for (let i = 0; i < foundRooms.length; i++) {
                ...
                console.log(
                    'findRoom: count users: ' + foundRooms[i].users.length
                );
                console.log('findRoom: topic: ' + foundRooms[i].topic);
                if (
                    foundRooms[i].topic === this.roomTopic &&
                    foundRooms[i].users.length < 4
                ) {
                    breakLoop = true;
    ...
    public static async getUsersSearching(): Promise<number> {
        const usersSearching = await db.User.count({
